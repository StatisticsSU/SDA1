...
plot(xgrid[xgrid.<=cutoff],normaldens[xgrid.<=cutoff], color=colors[6],
    ...
    xticks = ([0,cutoff],[0,L"z"]), xtickfontsize=22)
...
plot(xgrid[xgrid.<=cutoff],normaldens[xgrid.<=cutoff], color=colors[6],
    fill=(0,opacity,colors[6]), label = nothing, yaxis = false,
    xticks = ([-cutoff,0,cutoff],[L"-z",0,L"z"]), xtickfontsize=22, title = "Symmetri")
...
plot(xgrid[xgrid.>=cutoff],tdens[xgrid.>=cutoff], color=colors[2],
    ...
    xticks = ([0,cutoff],[0,L"t_{\alpha/2}"]), title = "Ensidigt", margins = 5mm)
...
plot(xgrid[xgrid.>=cutoff2],tdens[xgrid.>=cutoff2], color=colors[2],
    fill=(0,0.5,colors[2]), label = nothing, yaxis = false,
    xticks = ([cutoff1,0,cutoff2],[L"-t_{\alpha/2}",0,L"t_{\alpha/2}"]), title = "Tvåsidigt", margins = 5mm)
...
plot(xgrid[xgrid.>=cutoff], chi2dens[xgrid.>=cutoff], label = nothing, lw = 3,
    ylab = "", yticks = [], xticks = ([0, cutoff],[0, L"\chi^2_\alpha"]), xlab = "", c = colors[10],
...
plot(xgrid[xgrid.>=cutoff], Fdens[xgrid.>=cutoff], label = nothing, lw = 3,
    ylab = "", yticks = [], xticks = ([0,cutoff],[0,L"F_\alpha"]), xlab = "", c = colors[4],
...
annotate!(cutoff + 2.5, 0.095, 
    text(L"\alpha", :black, :right, 22))
plot!([cutoff + 2, cutoff + 1], [0.07,0.008], arrow = true, lw = 2, color = :black)
