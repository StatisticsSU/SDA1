...
Modellera variabeln `logcost = log(df$cost)` som oberoende observationer från en $N(\mu,\sigma)$ fördelning. Skatta $\mu$ och $\sigma$ från data.
...
Jämför median i data med medianen från den skattade modellen.

#### 💪 Uppgift 2.4

Gör ett 95%-igt konfidensintervall för $\mu$ , både genom att använda formellsamlingen och genom att använda en funktion i R. Tolka intervallet.

#### 💪 Uppgift 2.5

Testa om den genomsnittliga `logcost` i populationen är mindre än 6. Ställ upp noll- och alternativhypotes och testa på 5% signifikansnivå. Gör beräkningarna både med hjälp av formelsamlingen och med R.
...
där `logrooms = log(rooms)` och `logpeople = log(people)` (lägg till dessa transformerade variabler i vår dataframe `df`). Tolka skattningarna av koefficienterna $\beta_1$ och $\beta_5$.
...
Här är en bild från avhandlingen, där man ser den prediktiva sannolikhetsfördelningen för `cost` för olika värden på `logrooms`. Genom att utveckla en modell som kan modellera skevhet behövde vi inte transformera `cost` innan analysen, vilket blir trevligare att tolka.
