...
log_yhat_grid <- predict(lm_logfev_vs_height, newdata = new_x)
yhat_grid <- exp(log_yhat_grid) # Inverse transform
lines(FevChildren$height, y_hat, type = "p", col = "lightcoral") # Prediction of data points
...
- Vad kan man säga om sambandet mellan `SqFeet` och försälningspriset?
- Vad kan man säga om sambandet mellan `Lot` och försälningspriset?
- Verkar det finnas något samband försälningspriset och `Air`? Hur ska sambandet tolkas?
- En visning äger rum snart för en lägenhet som har en area på 2.311 kvadratfot, en tomtarea på 17.312 kvadratfot och aircondition och mäklaren undrar vad den kan säljas för. Kommunicera din prediktion med en tydlig tolkning.  
