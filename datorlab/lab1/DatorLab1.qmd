...
Läs återigen in data med `stockdata = read.xlsx('stock.xlsx', sheet = 1)`. Gör en graf där du plottar variabeln `capital` mot `year`.
...
Känns det bekant med \$-tecken för att plocka ut saker? Vi har redan sett att man kan plocka ut en kolumn i en dataframe (tabell) genom att skriva t ex `stockdata$capital` . En dataframe är nämligen egentligen en slags lista.
