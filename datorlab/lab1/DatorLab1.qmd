...
1.  Spara/flytta filen stock.xlsx till din kurskatalog som du skapade i del 3 ovan, dvs mappen **SDA1** i **Documents**. **Working directory** är den mapp som R kommer leta efter filer i. Ändra din arbetsmapp (**working directory**) i R till samma mapp **SDA1** i **Documents** genom att välja menyn [**S**]{.underline}**ession** och sen **Set [W]{.underline}orking Directory** och slutligen [**C**]{.underline}**hoose Directory...** och sedan klicka dig fram till mappen SDA1.
...
2.  För att slippa att klicka sig fram via menyer hela tiden så är det praktiskt att ändra working directory i början av den kodfil som man jobbar med. Då kan man bara köra den kodfilen och working directory ställs in automatiskt. Kommandot `setwd` gör samma sak som punkten 1 gjorde via menyerna. Här måste vi dock veta **sökvägen** (**path**) till mappen, dvs datorns sätt att hitta till (under)mappen **SDA1** i **Documents**-mappen. Sättet att skriva sökvägar på skiljer sig åt på Windows/Mac/Linux:
    ...
    -   ![](windows.png){width="15"} På en Windows-dator skriver kommandot `setwd('C:/Documents/SDA1')`. Notera de små 'blipparna' kring filvägen i `setwd` kommandot. \[Windows-sökvägar skrivs vanligtvis med backslash `\`. I RStudio skriver man ändå `/` för att matcha med Mac och Linux\].
...
där `read.xlsx` är en funktion som tre input-argument filnamn (som en sträng!), talet 1 för att vi vill läsa det första bladet och argumentet header sätts till TRUE eftersom första raden i Excelfilen innehåller namnen på kolumnerna. Funktionen `read.xlsx` ger tillbaka en dataframe som output med data.
...
## 11. Quarto: skriva rapporter med text, kod och figurer
